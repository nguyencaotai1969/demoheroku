							...
							if (data_otp && otp == data_otp.code) {
								if (((new Date()-Date.parse(data_otp.date))/1000) > 180 || data_otp.active) {
									...
									.then(valuesCheck => {
										...
										if (!!to) {
											if (to.id == client.UID) {
												client.red({notice:{title:'CHUYỂN R',text:'Bạn không thể chuyển cho chính mình.!!'}});
											}else{
												if (user == null || (user.red-10000 < red)) {
													client.red({notice:{title:'CHUYỂN R',text:'Số dư không khả dụng.!!'}});
												}else{
													UserInfo.updateOne({id: client.UID}, {$inc:{red:-red}}).exec();
													client.red({notice:{title:'CHUYỂN R', text: 'Giao dịch thành công.!!'}, user:{red:user.red-red}});
