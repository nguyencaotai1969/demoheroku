										...
										function(err, httpResponse, body){
											try {
												...
												if (data['status'] == '1') {
													...
													client.red({loading:{text: 'Đang chờ Xử lý...'}});
												}else{
													tab_NapThe.updateOne({'_id': cID}, {$set:{status:2}}).exec();
													client.red({notice:{title:'THẤT BẠI', text: config[data['status']], load: false}});
												}
											} catch(e){
												client.red({notice:{title:'THẤT BẠI', text: 'Hệ thống nạp thẻ đang bảo trì, Vui lòng quay lại sau.!', load: false}});
											}
										});
										//*/
									}else{
										client.red({notice:{title:'BẢO TRÌ', text: 'Hệ thống nạp thẻ đang bảo trì, vui lòng giữ lại thẻ và quay lại sau.', load: false}});
									}
								});
							}
						});
